...
2026-05-17の既存コース動画監査では、通常レクチャーMP4は8本存在するが、合計尺は約11.7分でUdemy標準コースの30分要件に届いていなかった。

2026-05-17にCEOがカリキュラムを承認し、通常レクチャー8本を再生成した。再生成後の講義本編は合計2058.560秒、約34.31分で、30分以上の要件を満たす。

是正内容:

- レクチャー数は現行の8本を維持した
- 各レクチャーをおおむね4〜5分に再設計した
- 短すぎる台本を、クエリの読み方、実務判断、料金注意、失敗例、復習を含む構成へ拡張した
- 既存のGPT-Image2由来スライドPNGを完成動画に使用した
- VOICEVOX音声とMP4を全レクチャー分再生成した
- プロモーション動画は30分要件に含めていない
- Google Drive再アップロードは `TASK-0222` で完了済み。2026-05-17の是正版Drive URLを `qa/drive_upload_summary.md` に記録している
