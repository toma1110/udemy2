...
- Completion Result: 2026-05-17時点で通常レクチャーMP4は6本、合計1961.926秒、約32.70分。全レクチャーをGoogle Driveへアップロード済み。

## Production Completion Gate

- 2026-05-17にCEO依頼に基づき全レクチャーの動画生成とDriveアップロードを完了した。
- プロモーション動画は通常レクチャーの30分要件に含めない。
- 標準範囲ではAWSリソースを作成していない。
- 完成動画のスライドと表示文字はGPT-Image2由来PNGに限定した。
...
| --- | --- | --- | --- | --- | --- |
| `s1-l1` | Runbookに何を書くか | Trigger、Owner、Severity、Scope、First Checks、Mitigation、Escalationを書く理由を説明できる | Runbookテンプレート記入演習 | 258.474秒 | Produced and uploaded |
| `s1-l2` | PlaybookとRunbookの違い | 調査手順としてのPlaybookと、既知の緩和・復旧手順としてのRunbookを区別できる | Playbook/Runbook比較表 | 315.188秒 | Produced and uploaded |
...
| Lecture ID | Lecture Title | Learning Goal | Hands-on Resource Title | Target Runtime | Production Status |
| --- | --- | --- | --- | --- | --- |
| `s2-l1` | CloudWatch Alarmから5分で確認すること | 影響範囲、対象サービス、直近変更、メトリクス、ログの初動確認を説明できる | 5分初動確認チェック | 339.434秒 | Produced and uploaded |
| `s2-l2` | Dashboard、Logs、Recent deploy、AWS Healthを見る | Dashboard、Logs、Recent deploy、AWS Healthを順番に見る理由を説明できる | Dashboard/Logs/AWS Health確認 | 332.564秒 | Produced and uploaded |
...
| Lecture ID | Lecture Title | Learning Goal | Hands-on Resource Title | Target Runtime | Production Status |
| --- | --- | --- | --- | --- | --- |
| `s3-l1` | Mitigation、Rollback、Escalationを決める | 一次緩和、ロールバック、権限外対応、エスカレーション条件を分けて書ける | Mitigation/Escalation設計 | 357.439秒 | Produced and uploaded |
| `s3-l2` | CommunicationとPostmortemへつなげる | 障害連絡、ステータス更新、ポストモーテムへの接続をRunbookへ入れられる | Communication/Postmortemテンプレート | 358.826秒 | Produced and uploaded |
...
- 通常レクチャー6本が存在する。
- 講義本編の合計動画尺が30分以上である。
- 2026-05-17に全レクチャーを生成し、既存の短尺 `s1-l1` だけを完成扱いにしない。
