...
## Production Scale Policy

この基準は、1本の動画を過剰に磨き続けるためではなく、量産時に最低限の品質を安定させるための基準です。

- GPT-Image2由来、文字の可読性、VOICEVOX誤読、技術的正確性、動画とREADMEの手順一致はBlockingとして扱う
- 上記を満たし、受講者レビューでハンズオン以外の重大指摘が出ていない場合は、微細な見た目改善より次の動画制作を優先する
- 初回動画で出た指摘は、その動画だけの修正で終わらせず、ルール、テンプレート、チェックリスト、スクリプトへ横展開する
- ハンズオンあり動画では、視覚品質よりもREADME再現性、Publicリポジトリ導線、CLI/図解/動画の整合を優先してレビューする
