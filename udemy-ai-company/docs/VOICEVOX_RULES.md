...
- 修正後は可能な限りWAVを聴取確認する
- CEOレビュー、受講者レビュー、QAで読み方指摘が出た語は、個別修正だけで終わらせず本ファイルと音声生成/チェック系ツールの置換表へ反映する
...
| 初学者 | しょがくしゃ |
| 空ではない、空の | からではない、からの |
| 触れる | 文脈に応じて、さわれる、または、ふれる |
| 方 | 文脈に応じて、かた、または、ほう |
| そんな方向け | そんなかたむけ |
| 目指す方、担当している方、進めたい方 | 目指すかた、担当しているかた、進めたいかた |
| 値を、値が、値に、値で | あたいを、あたいが、あたいに、あたいで |

`値` は `数値`、`目標値`、`閾値` など他の語に含まれるため、無条件に置換しません。助詞に続く単独の `値` だけを優先して直します。

`空` は文脈で読みが分かれます。ハンズオンの確認で「空ではない」「空の値」のように空欄や未設定を表す場合は `から` と読みます。空模様や空間を指す場合は対象外です。

`触れる` は文脈で読みが分かれます。ツールや画面を操作できる意味では `さわれる` と書き、概念に言及する意味では `ふれる` と書きます。

`方` も文脈で読みが分かれます。人を指す意味では `かた` と書き、方向や比較を指す意味では `ほう` と書きます。対象者を表す `そんな方向け` は、ナレーションでは `そんなかたむけ` と書きます。
...
読み方指摘を受けて台本を修正した場合は、同じ語が他講座に再発しないよう以下も確認します。

- `docs/VOICEVOX_RULES.md` の誤読しやすい和語テーブルに追加されているか
- `tools/narration_checker.py` の `JP_READING_FIXES` または `RISKY_JP_TERMS` に追加されているか
- 音声生成スクリプト側の読み替えテーブルにも必要な補正が入っているか
- 修正対象の `script.json` / `*_script.md` に同じ表記揺れが残っていないか
